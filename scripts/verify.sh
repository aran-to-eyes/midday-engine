...
# m1-project-new exit tests: `midday new <dir> --json` exits 0 on a fresh
# target; every generated file validates cleanly through validators the
# rest of the CLI already owns (midday.project.yaml / midday.import.yaml
# via the compiled-in schemas, core/loader/project_schema.h — NOT
# schema_manifest.json's formats[], that stays m1-scene-format's;
# default.input.yaml via the m1-input-actions extension dispatch; the empty
# scene via `midday run` — the M0 loader, never scene-format's schema,
# which does not exist yet); and the scaffold carries NO editor-private
# truth (asserted below: the file count on disk matches the envelope
# exactly, `.midday-cache/` does not exist until a CONSUMING verb creates
# one, and no generated byte is this checkout's own absolute path).
rm -rf build/dev/new_fixture
build/dev/midday new build/dev/new_fixture --json \
    | jq -e '.ok and (.files | length)==5 and .name=="new_fixture"' >/dev/null
build/dev/midday validate build/dev/new_fixture/midday.project.yaml --json \
    | jq -e '.ok and .schema=="project" and .format_version.authored==1' >/dev/null
build/dev/midday validate build/dev/new_fixture/midday.import.yaml --json \
    | jq -e '.ok and .schema=="import_policy" and .format_version.authored==1' >/dev/null
build/dev/midday validate build/dev/new_fixture/default.input.yaml --json \
    | jq -e '.ok and .schema=="input" and .actions==5 and .sticks==1' >/dev/null
build/dev/midday run build/dev/new_fixture/scenes/main.scene.yaml --json \
    | jq -e '.ok and .entities==0 and .ticks==0' >/dev/null
# Born canonical: every generated file is already `fmt`'s canonical form
# (built as a YamlNode tree through core/loader/yaml_emit.h, never a
# hand-indented string template).
for f in midday.project.yaml midday.import.yaml default.input.yaml scenes/main.scene.yaml; do
    build/dev/midday fmt "build/dev/new_fixture/$f" --check --json | jq -e '.ok and .canonical' >/dev/null
done
# A non-empty target refuses (exit 1, new.target_exists) instead of
# silently overwriting or partially merging into an existing project.
NEW_STATUS=0
NEW_OUT=$(build/dev/midday new build/dev/new_fixture --json) || NEW_STATUS=$?
[ "$NEW_STATUS" -eq 1 ]
echo "$NEW_OUT" | jq -e '.error.code == "new.target_exists"' >/dev/null
# No editor-private truth: exactly the reported files exist on disk (the
# three directories carry nothing else), no absolute path leaked into
# generated content, and nothing regenerable was ever written.
[ "$(find build/dev/new_fixture -type f | wc -l | tr -d ' ')" -eq 5 ]
