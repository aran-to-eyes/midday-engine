...
std::string emit_dts(const Json& document) {
    ...
    // Generator-neutral provenance on purpose: bootstrap and selfhost output
    // must stay byte-identical, so the header can never name the generator.
    std::string out = "// engine.d.ts -- GENERATED from engine_api.json. "
                      "DO NOT EDIT.\n// engine_version ";
