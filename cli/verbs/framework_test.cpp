...
TEST_CASE("cli.verbs: registry validates and resolves the manifest") {
    REQUIRE(midday::cli::verbs().size() == 15);
    ...
    CHECK(midday::cli::find_verb("run") == &midday::cli::run_spec());
    ...
    CHECK(midday::cli::find_verb("new") == &midday::cli::new_spec());
    CHECK(midday::cli::find_verb("scene") == &midday::cli::scene_spec());
    ...
    CHECK(unknown.payload.find("verbs")->dump() ==
          ...
          "\"shot\",\"validate\",\"fmt\",\"check\",\"mv\",\"new\",\"scene\"]");
}
